    ...
    async def build_profile(self, user_id: str, db: AsyncSession) -> dict:
        ...
        try:
            ...
            total_result = await db.execute(
                ...
                .where(Transaction.user_id == uid, Transaction.date >= ninety_days_ago)
            )
            ...
            category_result = await db.execute(
                ...
                .where(Transaction.user_id == uid, Transaction.date >= ninety_days_ago, Transaction.category != None)
                .group_by(Transaction.category)
                .order_by(func.sum(Transaction.amount).desc())
            )
            by_category = {row[0]: float(row[1] or 0) for row in category_result.all()}
            
            # 3. By Merchant (Top 10)
            merchant_result = await db.execute(
                select(Transaction.merchant, func.sum(Transaction.amount))
                .where(Transaction.user_id == uid, Transaction.date >= ninety_days_ago, Transaction.merchant != None)
                .group_by(Transaction.merchant)
                .order_by(func.sum(Transaction.amount).desc())
                .limit(10)
            )
            by_merchant = {row[0]: float(row[1] or 0) for row in merchant_result.all()}
            
            # 4. Monthly Totals
            month_key = func.to_char(Transaction.date, 'YYYY-MM')
            monthly_result = await db.execute(
                select(month_key, func.sum(Transaction.amount))
                .where(Transaction.user_id == uid, Transaction.date >= ninety_days_ago, Transaction.date != None)
                .group_by(month_key)
                .order_by(month_key)
            )
            monthly_totals = {row[0]: float(row[1] or 0) for row in monthly_result.all()}
            
            # 5. Recurring Merchants
            recurring_result = await db.execute(
                select(Transaction.merchant)
                .where(Transaction.user_id == uid, Transaction.merchant != None)
                .group_by(Transaction.merchant)
                .having(func.count() >= 2)
            )
            ...
            try:
                ...
                summary_text = "Could not generate AI summary due to an error, but your data is loaded."
